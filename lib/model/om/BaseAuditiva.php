...
abstract class BaseAuditiva extends BaseObject  implements Persistent {


  const PEER = 'AuditivaPeer';
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = AuditivaPeer::ID;
		}
	...
	public function setFkEstablecimientoId($v)
	{
		...
		if ($this->fk_establecimiento_id !== $v || $v === 0) {
			...
			$this->modifiedColumns[] = AuditivaPeer::FK_ESTABLECIMIENTO_ID;
		}
	...
	public function setFkCarreraId($v)
	{
		...
		if ($this->fk_carrera_id !== $v || $v === 0) {
			...
			$this->modifiedColumns[] = AuditivaPeer::FK_CARRERA_ID;
		}
	...
	public function setNombre($v)
	{
		...
		if ($this->nombre !== $v) {
			$this->nombre = $v;
			$this->modifiedColumns[] = AuditivaPeer::NOMBRE;
		}
	...
	public function setDescripcion($v)
	{
		...
		if ($this->descripcion !== $v) {
			$this->descripcion = $v;
			$this->modifiedColumns[] = AuditivaPeer::DESCRIPCION;
		}
	...
	public function setOrden($v)
	{
		...
		if ($this->orden !== $v) {
			$this->orden = $v;
			$this->modifiedColumns[] = AuditivaPeer::ORDEN;
		}
	...
	public function setAnio($v)
	{
		...
		if ($this->anio !== $v) {
			$this->anio = $v;
			$this->modifiedColumns[] = AuditivaPeer::ANIO;
		}
	...
	{
						if (array_diff($this->modifiedColumns, array(AuditivaPeer::FK_ESTABLECIMIENTO_ID,AuditivaPeer::FK_CARRERA_ID))) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			throw new PropelException("Error populating Auditiva object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = AuditivaPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			AuditivaPeer::doDelete($this, $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			$con->commit();
			AuditivaPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
		...
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			...
			if ($this->isNew() ) {
				$this->modifiedColumns[] = AuditivaPeer::ID;
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = AuditivaPeer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += AuditivaPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = AuditivaPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = AuditivaPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = AuditivaPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AuditivaPeer::ID)) $criteria->add(AuditivaPeer::ID, $this->id);
		if ($this->isColumnModified(AuditivaPeer::FK_ESTABLECIMIENTO_ID)) $criteria->add(AuditivaPeer::FK_ESTABLECIMIENTO_ID, $this->fk_establecimiento_id);
		if ($this->isColumnModified(AuditivaPeer::FK_CARRERA_ID)) $criteria->add(AuditivaPeer::FK_CARRERA_ID, $this->fk_carrera_id);
		if ($this->isColumnModified(AuditivaPeer::NOMBRE)) $criteria->add(AuditivaPeer::NOMBRE, $this->nombre);
		if ($this->isColumnModified(AuditivaPeer::DESCRIPCION)) $criteria->add(AuditivaPeer::DESCRIPCION, $this->descripcion);
		if ($this->isColumnModified(AuditivaPeer::ORDEN)) $criteria->add(AuditivaPeer::ORDEN, $this->orden);
		if ($this->isColumnModified(AuditivaPeer::ANIO)) $criteria->add(AuditivaPeer::ANIO, $this->anio);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(AuditivaPeer::ID, $this->id);
