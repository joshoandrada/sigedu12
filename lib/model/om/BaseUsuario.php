...
abstract class BaseUsuario extends BaseObject  implements Persistent {
  ...
  const PEER = 'UsuarioPeer';
	...
	public function setClave2($v)
	{
		...
		if ($this->clave2 !== $v) {
			$this->clave2 = $v;
			$this->modifiedColumns[] = UsuarioPeer::CLAVE2;
		}

		return $this;
	} 
	
	public function setFkAlumno($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->fk_alumno !== $v || $v === 0) {
			$this->fk_alumno = $v;
			$this->modifiedColumns[] = UsuarioPeer::FK_ALUMNO;
		}
	...
	public function setNroDocumento($v)
	{
		...
		if ($this->nro_documento !== $v) {
			...
			$this->modifiedColumns[] = UsuarioPeer::NRO_DOCUMENTO;
		}
	...
	{
						if (array_diff($this->modifiedColumns, array(UsuarioPeer::CORREO_PUBLICO,UsuarioPeer::ACTIVO,UsuarioPeer::FK_ALUMNO,UsuarioPeer::FK_ESTABLECIMIENTO_ID,UsuarioPeer::BORRADO))) {
				return false;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->clave2 = ($row[$startcol + 9] !== null) ? (string) $row[$startcol + 9] : null;
			$this->fk_alumno = ($row[$startcol + 10] !== null) ? (int) $row[$startcol + 10] : null;
			$this->email = ($row[$startcol + 11] !== null) ? (string) $row[$startcol + 11] : null;
			$this->fk_establecimiento_id = ($row[$startcol + 12] !== null) ? (int) $row[$startcol + 12] : null;
			$this->borrado = ($row[$startcol + 13] !== null) ? (boolean) $row[$startcol + 13] : null;
	...
	public function ensureConsistency()
	{
		...
		if ($this->aAlumno !== null && $this->fk_alumno !== $this->aAlumno->getId()) {
			$this->aAlumno = null;
		}
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			...
			case 8:
				...
				break;
			case 9:
				return $this->getClave2();
				break;
			case 10:
				return $this->getFkAlumno();
				break;
			case 11:
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[9] => $this->getClave2(),
			$keys[10] => $this->getFkAlumno(),
			$keys[11] => $this->getEmail(),
			$keys[12] => $this->getFkEstablecimientoId(),
			$keys[13] => $this->getBorrado(),
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			...
			case 8:
				...
				break;
			case 9:
				$this->setClave2($value);
				break;
			case 10:
				$this->setFkAlumno($value);
				break;
			case 11:
				$this->setEmail($value);
				break;
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[9], $arr)) $this->setClave2($arr[$keys[9]]);
		if (array_key_exists($keys[10], $arr)) $this->setFkAlumno($arr[$keys[10]]);
		if (array_key_exists($keys[11], $arr)) $this->setEmail($arr[$keys[11]]);
		if (array_key_exists($keys[12], $arr)) $this->setFkEstablecimientoId($arr[$keys[12]]);
		if (array_key_exists($keys[13], $arr)) $this->setBorrado($arr[$keys[13]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(UsuarioPeer::CLAVE2)) $criteria->add(UsuarioPeer::CLAVE2, $this->clave2);
		if ($this->isColumnModified(UsuarioPeer::FK_ALUMNO)) $criteria->add(UsuarioPeer::FK_ALUMNO, $this->fk_alumno);
		...
		if ($this->isColumnModified(UsuarioPeer::NRO_DOCUMENTO)) $criteria->add(UsuarioPeer::NRO_DOCUMENTO, $this->nro_documento);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setClave2($this->clave2);

		$copyObj->setFkAlumno($this->fk_alumno);
	...
	{
		if ($this->aAlumno === null && ($this->fk_alumno !== null)) {
			$c = new Criteria(AlumnoPeer::DATABASE_NAME);
			$c->add(AlumnoPeer::ID, $this->fk_alumno);
