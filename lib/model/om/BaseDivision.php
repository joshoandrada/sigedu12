...
abstract class BaseDivision extends BaseObject  implements Persistent {
  ...
  const PEER = 'DivisionPeer';
		...
		if ($this->collRelactividaddivisions === null) {
			if ($this->isNew()) {
			   $this->collRelactividaddivisions = array();
				...
				$criteria->add(RelactividaddivisionPeer::FK_DIVISION_ID, $this->id);
	...
	public function getRelactividaddivisionsJoinActividad($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(DivisionPeer::DATABASE_NAME);
		}
		...
		if ($this->collRelactividaddivisions === null) {
			if ($this->isNew()) {
				...
				$this->collRelactividaddivisions = RelactividaddivisionPeer::doSelectJoinActividad($criteria, $con, $join_behavior);
			}
		} else {
