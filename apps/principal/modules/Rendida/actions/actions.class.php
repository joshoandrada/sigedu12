 ...
 */
class RendidaActions extends autoRendidaActions
{
...
$ii=0;
       ...
       foreach ($a1 as $aa1)
       {
               ...
               $folio = $aa1->getFolio();
               $fech= $aa1->getFecha();
               $fecha6 = date('d',strtotime($fech))."/".date('n',strtotime($fech))."/".date('Y',strtotime($fech)); 
...
$pdf->Output('fichaAlumno.pdf', 'I');
 throw new sfStopException();
  ...
  {
    return $this->forward('Rendida', 'list');
  }
  ...
  {
    return $this->forward('Rendida', 'edit');
  }
  ...
  public function executeDeleteSelected($request)
  {
    ...
    return $this->redirect('Rendida/list');
  }
  ...
  public function executeEdit($request)
  {
    ...
    if ($request->isMethod('post'))
    {
      ...
      {
        return $this->redirect('Rendida/edit?id='.$this->rendida->getId());
      }
  ...
  protected function addFiltersCriteria($c)
  {
    ...
    {
      $criterion = $c->getNewCriterion(RendidaPeer::FK_ALUMNO_ID, '');
      $criterion->addOr($c->getNewCriterion(RendidaPeer::FK_ALUMNO_ID, null, Criteria::ISNULL));
      $c->add($criterion);
    }
    else if (isset($this->filters['fk_alumno_id']) && $this->filters['fk_alumno_id'] !== '')
    {
      $c->add(RendidaPeer::FK_ALUMNO_ID, $this->filters['fk_alumno_id']);
    }
...
$rend=$this->getRequestParameter('idr');
    ...
    return $this->redirect('Rendida/edit?id='.$rend);
