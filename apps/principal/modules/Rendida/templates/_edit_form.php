<?php echo form_tag('Rendida/save', array(
		...
		$i8[$e8++]=$cc->getFolio();
                ...
                if (empty($fc)){
                 $i9[$e9++]=$fc;
                  ...
                  {
                  $i9[$e9++]= date("d-m-Y",strtotime($cc->getFecha()));
                  };

                //$fc = date("d-m-Y",strtotime($cc->getFecha()));
...
<div class="form-row">
    <?php include_partial('Rendida/verdetalles', array('num' => $num,'dato1' => $dato1, 'datos'=>$datos,'dato2' => $dato2,'dato4' => $dato4,'dato5' => $dato5,'dato6' => $dato6,'dato7' => $dato7,'dato8' => $dato8,'dato9' => $dato9,'idrendida'=>$rendida->getId(),'plan'=>$canio)) ?>
</div>
</fieldset>
...
<div class="form-row1">
<div class="col2">
<p style="color: #FF0000;"><?php  echo  "Aún no ha guardado Rendida..."; ?></p>
</div>
</div>
...
<? if($num === '' || $num === null || $num ===0){?>
 ...
 <ul class="sf_admin_actions">
   <li><?php echo button_to(('Cambiar Alumno '),'buscaalumno1/list?idrendida='.$rendida->getId(), array (
...
<? if($alucarre === 7){?>
<ul class="sf_admin_actions">
      <li><?php echo button_to(('Cargar Materias y nota'), 'bcomun1a/list?idrendida='.$rendida->getId().'&ida='.$nomape, array ( 
...
<? if($alucarre === 10){?>
<ul class="sf_admin_actions">
      <li><?php echo button_to(('Cargar Materias y nota'), 'bneuro1a/list?idrendida='.$rendida->getId().'&ida='.$nomape, array ( 
  'class' => 'sf_admin_action_cuenta1',
))?></li>
    </ul>
<?}?>
...
<? if($alucarre === 11){?>
<ul class="sf_admin_actions">
      <li><?php echo button_to(('Cargar Materias y nota'), 'bauditiva1a/list?idrendida='.$rendida->getId().'&ida='.$nomape, array ( 
  'class' => 'sf_admin_action_cuenta1',
))?></li>
    </ul>
<?}?>
...
<? if($alucarre === 12){?>
<ul class="sf_admin_actions">
      <li><?php echo button_to(('Cargar Materias y nota'), 'bintel1a/list?idrendida='.$rendida->getId().'&ida='.$nomape, array ( 
  'class' => 'sf_admin_action_cuenta1',
))?></li>
    </ul>
<?}?>
...
<? if($alucarre === 13){?>
<ul class="sf_admin_actions">
      <li><?php echo button_to(('Cargar Materias y nota'), 'bvisual1a/list?idrendida='.$rendida->getId().'&ida='.$nomape,  array ( 
  'class' => 'sf_admin_action_cuenta1',
))?></li>
    </ul>
<?}?>
...
<?php echo button_to(__('Borrar'), 'Rendida/delete?id='.$rendida->getId(), array (
  'post' => true,
