...
class usuarioActions extends autousuarioActions
{
    ...
    function updateUsuarioFromRequest() {
        ...
        if (isset($usuario['nro_documento'])) {
          $this->usuario->setNroDocumento($usuario['nro_documento']);
        }
 ...
 protected function buscarDatos($usuario)
{

if ($usuario->getFkAlumno() == 0 || $usuario->getFkAlumno() == "" )
    {
     $h2= sfPropelFinder::from('Alumno')->
     where ('NroDocumento','like',$usuario->getNroDocumento())->
     find();
     foreach ($h2 as $h31)  
       {        
	$id=$h31->getId();
       ...
       if ($id != 0 || $id != "")
       { 
       $h= sfPropelFinder::from('Usuario')->
       where ('Id','like',$usuario->getId())->
       find();
       foreach ($h as $css)  
         {        
	 $css->setFkAlumno($id);
         ...
         $rec1 = new UsuarioRol();
         $rec1->setFkUsuarioId($usuario->getId());
         $rec1->setFkRolId(4);
         $rec1->save();
       }
       else {
          $rec1 = new Alumno();
          $rec1->setNroDocumento($usuario->getNroDocumento());
          $rec1->setFKProvinciaId(1);
          $rec1->setFkTipodocumentoId(2);
          $rec1->setFkCarreraId(7);
          $rec1->setFkPaisId(1);
          $rec1->save();

          $h21= sfPropelFinder::from('Alumno')->
          where ('NroDocumento','like',$usuario->getNroDocumento())->
          find();
          foreach ($h21 as $h32)  
            {        
	    $id1=$h32->getId();
          ...
          foreach ($h1 as $css1)  
             {        
	     $css1->setFkAlumno($id1);
             $css1->save();
             }

         $rec2 = new UsuarioRol();
         $rec2->setFkUsuarioId($usuario->getId());
         $rec2->setFkRolId(4);
         $rec2->save();
         }
     }
