<div id="sf_admin_container">
    ...
    function linkTo() {
        ...
        location.href = "<?php echo url_for('usuarioa/editPermiso', false);?>/id/<?php echo $sf_params->get('id');?>/rolId/"+obj.options[obj.selectedIndex].value;
    }
	</script>
<div id="sf_admin_content">
  ...
  <br/>
  <h2>Roles:</h2>
  <?php echo form_tag('usuarioa/saveRol')?>
      ...
      <?php if(count($usuarioroles)> 0):?>
        ...
        <?php foreach ($usuarioroles as $rol):?>
          <tr class="sf_admin_row_<?php echo $row?>">
            ...
            <td><?php echo link_to(image_tag('../sf/sf_admin/images/delete_icon.png',array('alt'=>'Eliminar rol', 'title'=>'Eliminar rol')),'usuarioa/deleteRol?usuario_id='.$usuario->getId().'&rol_id=' . $rol->getRol()->getId(),'confirm=Eliminar el rol del usuario?')?></td>
          </tr>
          <?php $row = ! $row;?>
        <?php endforeach;?>
  ...
  <h2>Permisos</h2>
    <p>
      <?php echo form_tag('usuarioa/savePermiso', 'onSubmit="selectItem()"')?>
      <?php
      ...
      <li><?php echo button_to('Listado de usuarios','usuarioa/list',array('class'=>'sf_admin_action_list'))?></li>
      </ul>
      </form>
    </p>
